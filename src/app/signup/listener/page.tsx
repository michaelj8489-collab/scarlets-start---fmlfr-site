...
export default async function ListenerSignupPage({ searchParams }: { searchParams: Promise<{ message?: string }> }) {
  ...
  return (
    <main className="flex min-h-[calc(100svh-73px)] items-center justify-center bg-black px-4 py-10">
      <section className="w-full max-w-md rounded-2xl border border-scarlet/30 bg-black/70 p-6 shadow-[0_0_40px_rgba(193,18,31,0.22)] sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-scarlet">Free Listener</p>
        ...
        <form className="mt-7 flex flex-col gap-5">
          <input type="hidden" name="accountType" value="listener" />
        ...
        <p className="mt-6 text-center text-sm text-gray-400">
          Already a member? <Link href="/login" className="text-scarlet hover:text-white">Login</Link>
        </p>
      </section>
    </main>
  )
